...
class BaseResolver:
	...
	def add_implicit_resolver(cls,tag,regexp,first):
		if'yaml_implicit_resolvers'not in cls.__dict__:
			implicit_resolvers={}
			for key in cls.yaml_implicit_resolvers:implicit_resolvers[key]=cls.yaml_implicit_resolvers[key][:]
	...
	def add_path_resolver(cls,tag,path,kind=_A):
		...
		for element in path:
			...
			elif node_check not in[ScalarNode,SequenceNode,MappingNode]and not isinstance(node_check,str)and node_check is not _A:raise ResolverError('Invalid node checker: %s'%node_check)
			if not isinstance(index_check,(str,int))and index_check is not _A:raise ResolverError('Invalid index checker: %s'%index_check)
	...
	def check_resolver_prefix(self,depth,path,kind,current_node,current_index):
		...
		if isinstance(node_check,str):
			...
			if not isinstance(current_node,node_check):return
		if index_check is True and current_index is not _A:return
		if(index_check is False or index_check is _A)and current_index is _A:return
		if isinstance(index_check,str):
			if not(isinstance(current_index,ScalarNode)and index_check==current_index.value):return
	...
	def resolve(self,kind,value,implicit):
		if kind is ScalarNode and implicit[0]:
			...
			wildcard_resolvers=self.yaml_implicit_resolvers.get(_A,[])
