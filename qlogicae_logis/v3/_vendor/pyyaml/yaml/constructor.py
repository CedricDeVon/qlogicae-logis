_S='expected the empty value, but found %r'
_R='module %r is not imported'
_Q='cannot find module %r (%s)'
_P='expected non-empty name appended to the tag'
_O='expected a single mapping item, but found %d items'
_N='expected a mapping of length 1, but found %s'
_M='expected a sequence, but found %s'
_L='failed to decode base64 data: %s'
_K='decodebytes'
_J='failed to convert base64 data into ascii: %s'
_I='tag:yaml.org,2002:str'
_H='tag:yaml.org,2002:value'
_G='expected a mapping node, but found %s'
_F='__setstate__'
_E='while constructing a mapping'
_D='while constructing a Python module'
_C=True
_B=False
_A=None
__all__=['BaseConstructor','SafeConstructor','FullConstructor','UnsafeConstructor','Constructor','ConstructorError']
import base64,binascii,collections.abc,datetime,re,sys,types
from.error import*
from.nodes import*
class ConstructorError(MarkedYAMLError):0
class BaseConstructor:
	yaml_constructors={};yaml_multi_constructors={}
	def __init__(self):self.constructed_objects={};self.recursive_objects={};self.state_generators=[];self.deep_construct=_B
	def check_data(self):return self.check_node()
	def check_state_key(self,key):
		if self.get_state_keys_blacklist_regexp().match(key):raise ConstructorError(_A,_A,"blacklisted key '%s' in instance state found"%(key,),_A)
	def get_data(self):
		if self.check_node():return self.construct_document(self.get_node())
	...
	def construct_object(self,node,deep=_B):
		...
		if node in self.recursive_objects:raise ConstructorError(_A,_A,'found unconstructable recursive node',node.start_mark)
		self.recursive_objects[node]=_A;constructor=_A;tag_suffix=_A
		if node.tag in self.yaml_constructors:constructor=self.yaml_constructors[node.tag]
		else:
			for tag_prefix in self.yaml_multi_constructors:
				if tag_prefix is not _A and node.tag.startswith(tag_prefix):tag_suffix=node.tag[len(tag_prefix):];constructor=self.yaml_multi_constructors[tag_prefix];break
			else:
				if _A in self.yaml_multi_constructors:tag_suffix=node.tag;constructor=self.yaml_multi_constructors[_A]
		...
		self.constructed_objects[node]=data;del self.recursive_objects[node]
		if deep:self.deep_construct=old_deep
		return data
	def construct_scalar(self,node):
		if not isinstance(node,ScalarNode):raise ConstructorError(_A,_A,'expected a scalar node, but found %s'%node.id,node.start_mark)
		return node.value
	def construct_sequence(self,node,deep=_B):
		if not isinstance(node,SequenceNode):raise ConstructorError(_A,_A,'expected a sequence node, but found %s'%node.id,node.start_mark)
		return[self.construct_object(child,deep=deep)for child in node.value]
	def construct_mapping(self,node,deep=_B):
		if not isinstance(node,MappingNode):raise ConstructorError(_A,_A,_G%node.id,node.start_mark)
		mapping={}
		for(key_node,value_node)in node.value:
			key=self.construct_object(key_node,deep=deep)
			if not isinstance(key,collections.abc.Hashable):raise ConstructorError(_E,node.start_mark,'found unhashable key',key_node.start_mark)
			value=self.construct_object(value_node,deep=deep);mapping[key]=value
		return mapping
...
class SafeConstructor(BaseConstructor):
	def construct_scalar(self,node):
		if isinstance(node,MappingNode):
			for(key_node,value_node)in node.value:
				if key_node.tag==_H:return self.construct_scalar(value_node)
			...
			if key_node.tag=='tag:yaml.org,2002:merge':
				...
				if isinstance(value_node,MappingNode):self.flatten_mapping(value_node);merge.extend(value_node.value)
					...
					for subnode in value_node.value:
						if not isinstance(subnode,MappingNode):raise ConstructorError(_E,node.start_mark,'expected a mapping for merging, but found %s'%subnode.id,subnode.start_mark)
						self.flatten_mapping(subnode);submerge.append(subnode.value)
					submerge.reverse()
					for value in submerge:merge.extend(value)
				else:raise ConstructorError(_E,node.start_mark,'expected a mapping or list of mappings for merging, but found %s'%value_node.id,value_node.start_mark)
			elif key_node.tag==_H:key_node.tag=_I;index+=1
			else:index+=1
		if merge:node.value=merge+node.value
	def construct_mapping(self,node,deep=_B):
	...
	bool_values={'yes':_C,'no':_B,'true':_C,'false':_B,'on':_C,'off':_B}
	def construct_yaml_bool(self,node):value=self.construct_scalar(node);return self.bool_values[value.lower()]
	def construct_yaml_int(self,node):
		value=self.construct_scalar(node);value=value.replace('_','');sign=1
		if value[0]=='-':sign=-1
		if value[0]in'+-':value=value[1:]
		if value=='0':return 0
		elif value.startswith('0b'):return sign*int(value[2:],2)
		elif value.startswith('0x'):return sign*int(value[2:],16)
		elif value[0]=='0':return sign*int(value,8)
		elif':'in value:
			digits=[int(part)for part in value.split(':')];digits.reverse();base=1;value=0
			for digit in digits:value+=digit*base;base*=60
			return sign*value
		else:return sign*int(value)
	inf_value=1e300
	while inf_value!=inf_value*inf_value:inf_value*=inf_value
	nan_value=-inf_value/inf_value
	def construct_yaml_float(self,node):
		value=self.construct_scalar(node);value=value.replace('_','').lower();sign=1
		if value[0]=='-':sign=-1
		if value[0]in'+-':value=value[1:]
		if value=='.inf':return sign*self.inf_value
		elif value=='.nan':return self.nan_value
		elif':'in value:
			digits=[float(part)for part in value.split(':')];digits.reverse();base=1;value=.0
			for digit in digits:value+=digit*base;base*=60
			return sign*value
		else:return sign*float(value)
	def construct_yaml_binary(self,node):
		try:value=self.construct_scalar(node).encode('ascii')
		except UnicodeEncodeError as exc:raise ConstructorError(_A,_A,_J%exc,node.start_mark)
		try:
			if hasattr(base64,_K):return base64.decodebytes(value)
			else:return base64.decodestring(value)
		except binascii.Error as exc:raise ConstructorError(_A,_A,_L%exc,node.start_mark)
	timestamp_regexp=re.compile('^(?P<year>[0-9][0-9][0-9][0-9])\n                -(?P<month>[0-9][0-9]?)\n                -(?P<day>[0-9][0-9]?)\n                (?:(?:[Tt]|[ \\t]+)\n                (?P<hour>[0-9][0-9]?)\n                :(?P<minute>[0-9][0-9])\n                :(?P<second>[0-9][0-9])\n                (?:\\.(?P<fraction>[0-9]*))?\n                (?:[ \\t]*(?P<tz>Z|(?P<tz_sign>[-+])(?P<tz_hour>[0-9][0-9]?)\n                (?::(?P<tz_minute>[0-9][0-9]))?))?)?$',re.X)
	def construct_yaml_timestamp(self,node):
		C='tz_sign';B='fraction';A='hour';value=self.construct_scalar(node);match=self.timestamp_regexp.match(node.value);values=match.groupdict();year=int(values['year']);month=int(values['month']);day=int(values['day'])
		if not values[A]:return datetime.date(year,month,day)
		hour=int(values[A]);minute=int(values['minute']);second=int(values['second']);fraction=0;tzinfo=_A
		if values[B]:
			fraction=values[B][:6]
			while len(fraction)<6:fraction+='0'
			fraction=int(fraction)
		if values[C]:
			tz_hour=int(values['tz_hour']);tz_minute=int(values['tz_minute']or 0);delta=datetime.timedelta(hours=tz_hour,minutes=tz_minute)
			if values[C]=='-':delta=-delta
			tzinfo=datetime.timezone(delta)
		elif values['tz']:tzinfo=datetime.UTC
		return datetime.datetime(year,month,day,hour,minute,second,fraction,tzinfo=tzinfo)
	def construct_yaml_omap(self,node):
		A='while constructing an ordered map';omap=[];yield omap
		if not isinstance(node,SequenceNode):raise ConstructorError(A,node.start_mark,_M%node.id,node.start_mark)
		for subnode in node.value:
			if not isinstance(subnode,MappingNode):raise ConstructorError(A,node.start_mark,_N%subnode.id,subnode.start_mark)
			if len(subnode.value)!=1:raise ConstructorError(A,node.start_mark,_O%len(subnode.value),subnode.start_mark)
			key_node,value_node=subnode.value[0];key=self.construct_object(key_node);value=self.construct_object(value_node);omap.append((key,value))
	...
	def construct_yaml_object(self,node,cls):
		...
		if hasattr(data,_F):state=self.construct_mapping(node,deep=_C);data.__setstate__(state)
		else:state=self.construct_mapping(node);data.__dict__.update(state)
	def construct_undefined(self,node):raise ConstructorError(_A,_A,'could not determine a constructor for the tag %r'%node.tag,node.start_mark)
...
SafeConstructor.add_constructor(_I,SafeConstructor.construct_yaml_str)
...
class FullConstructor(SafeConstructor):
	def get_state_keys_blacklist(self):return['^extend$','^__.*__$']
	...
	def construct_python_long(self,node):return self.construct_yaml_int(node)
	...
	def find_python_module(self,name,mark,unsafe=_B):
		if not name:raise ConstructorError(_D,mark,_P,mark)
		if unsafe:
			try:__import__(name)
			except ImportError as exc:raise ConstructorError(_D,mark,_Q%(name,exc),mark)
		if name not in sys.modules:raise ConstructorError(_D,mark,_R%name,mark)
		return sys.modules[name]
	def find_python_name(self,name,mark,unsafe=_B):
		A='while constructing a Python object'
		if not name:raise ConstructorError(A,mark,_P,mark)
		if'.'in name:module_name,object_name=name.rsplit('.',1)
		else:module_name='builtins';object_name=name
		if unsafe:
			try:__import__(module_name)
			except ImportError as exc:raise ConstructorError(A,mark,_Q%(module_name,exc),mark)
		if module_name not in sys.modules:raise ConstructorError(A,mark,_R%module_name,mark)
		module=sys.modules[module_name]
		if not hasattr(module,object_name):raise ConstructorError(A,mark,'cannot find %r in the module %r'%(object_name,module.__name__),mark)
		return getattr(module,object_name)
	def construct_python_name(self,suffix,node):
		value=self.construct_scalar(node)
		if value:raise ConstructorError('while constructing a Python name',node.start_mark,_S%value,node.start_mark)
		return self.find_python_name(suffix,node.start_mark)
	def construct_python_module(self,suffix,node):
		value=self.construct_scalar(node)
		if value:raise ConstructorError(_D,node.start_mark,_S%value,node.start_mark)
		return self.find_python_module(suffix,node.start_mark)
	def make_python_instance(self,suffix,node,args=_A,kwds=_A,newobj=_B,unsafe=_B):
		if not args:args=[]
		if not kwds:kwds={}
		cls=self.find_python_name(suffix,node.start_mark)
		if not(unsafe or isinstance(cls,type)):raise ConstructorError('while constructing a Python instance',node.start_mark,'expected a class, but found %r'%type(cls),node.start_mark)
		if newobj and isinstance(cls,type):return cls.__new__(cls,*args,**kwds)
		else:return cls(*args,**kwds)
	def set_python_instance_state(self,instance,state,unsafe=_B):
		if hasattr(instance,_F):instance.__setstate__(state)
		else:
			slotstate={}
			if isinstance(state,tuple)and len(state)==2:state,slotstate=state
			if hasattr(instance,'__dict__'):
				if not unsafe and state:
					for key in state.keys():self.check_state_key(key)
				instance.__dict__.update(state)
			elif state:slotstate.update(state)
			for(key,value)in slotstate.items():
				if not unsafe:self.check_state_key(key)
				setattr(instance,key,value)
	def construct_python_object(self,suffix,node):instance=self.make_python_instance(suffix,node,newobj=_C);yield instance;deep=hasattr(instance,_F);state=self.construct_mapping(node,deep=deep);self.set_python_instance_state(instance,state)
	def construct_python_object_apply(self,suffix,node,newobj=_B):
		...
		if state:self.set_python_instance_state(instance,state)
		if listitems:instance.extend(listitems)
		if dictitems:
...
class UnsafeConstructor(FullConstructor):
	def find_python_module(self,name,mark):return super().find_python_module(name,mark,unsafe=_C)
	def find_python_name(self,name,mark):return super().find_python_name(name,mark,unsafe=_C)
	def make_python_instance(self,suffix,node,args=_A,kwds=_A,newobj=_B):return super().make_python_instance(suffix,node,args,kwds,newobj,unsafe=_C)
	def set_python_instance_state(self,instance,state):return super().set_python_instance_state(instance,state,unsafe=_C)
...
class Constructor(UnsafeConstructor):0