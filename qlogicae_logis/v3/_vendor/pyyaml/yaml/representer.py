_E='cannot represent an object'
_D='tag:yaml.org,2002:timestamp'
_C=False
_B=True
_A=None
__all__=['BaseRepresenter','SafeRepresenter','Representer','RepresenterError']
import base64,collections,copyreg,datetime,types
from.error import*
from.nodes import*
class RepresenterError(YAMLError):0
class BaseRepresenter:
	yaml_representers={};yaml_multi_representers={}
	def __init__(self,default_style=_A,default_flow_style=_C,sort_keys=_B):self.default_style=default_style;self.sort_keys=sort_keys;self.default_flow_style=default_flow_style;self.represented_objects={};self.object_keeper=[];self.alias_key=_A
	def represent(self,data):node=self.represent_data(data);self.serialize(node);self.represented_objects={};self.object_keeper=[];self.alias_key=_A
	def represent_data(self,data):
		...
		if data_types[0]in self.yaml_representers:node=self.yaml_representers[data_types[0]](self,data)
			...
			for data_type in data_types:
				...
				else:node=ScalarNode(_A,str(data))
		return node
	@classmethod
	...
	def represent_scalar(self,tag,value,style=_A):
	...
	def represent_mapping(self,tag,mapping,flow_style=_A):
		...
		for(item_key,item_value)in mapping:
			node_key=self.represent_data(item_key);node_value=self.represent_data(item_value)
...
class SafeRepresenter(BaseRepresenter):
	def ignore_aliases(self,data):
		if data is _A:return _B
		if isinstance(data,tuple)and data==():return _B
		if isinstance(data,(str,bytes,bool,int,float)):return _B
	def represent_none(self,data):return self.represent_scalar('tag:yaml.org,2002:null','null')
	def represent_str(self,data):return self.represent_scalar('tag:yaml.org,2002:str',data)
	def represent_binary(self,data):
		A='ascii'
		if hasattr(base64,'encodebytes'):data=base64.encodebytes(data).decode(A)
		else:data=base64.encodestring(data).decode(A)
		return self.represent_scalar('tag:yaml.org,2002:binary',data,style='|')
	def represent_bool(self,data):
		if data:value='true'
		else:value='false'
		return self.represent_scalar('tag:yaml.org,2002:bool',value)
	def represent_int(self,data):return self.represent_scalar('tag:yaml.org,2002:int',str(data))
	inf_value=1e300
	...
	def represent_list(self,data):return self.represent_sequence('tag:yaml.org,2002:seq',data)
	def represent_dict(self,data):return self.represent_mapping('tag:yaml.org,2002:map',data)
	def represent_set(self,data):
		value={}
		for key in data:value[key]=_A
		return self.represent_mapping('tag:yaml.org,2002:set',value)
	def represent_date(self,data):value=data.isoformat();return self.represent_scalar(_D,value)
	def represent_datetime(self,data):value=data.isoformat(' ');return self.represent_scalar(_D,value)
	def represent_yaml_object(self,tag,data,cls,flow_style=_A):
		if hasattr(data,'__getstate__'):state=data.__getstate__()
		else:state=data.__dict__.copy()
		return self.represent_mapping(tag,state,flow_style=flow_style)
	def represent_undefined(self,data):raise RepresenterError(_E,data)
...
class Representer(SafeRepresenter):
	...
	def represent_name(self,data):name='%s.%s'%(data.__module__,data.__name__);return self.represent_scalar('tag:yaml.org,2002:python/name:'+name,'')
	...
	def represent_object(self,data):
		...
		else:raise RepresenterError(_E,data)
		...
		if function.__name__=='__newobj__':function=args[0];args=args[1:];tag='tag:yaml.org,2002:python/object/new:';newobj=_B
		else:tag='tag:yaml.org,2002:python/object/apply:';newobj=_C
		function_name='%s.%s'%(function.__module__,function.__name__)
		if not args and not listitems and not dictitems and isinstance(state,dict)and newobj:return self.represent_mapping('tag:yaml.org,2002:python/object:'+function_name,state)
	...
	def represent_ordered_dict(self,data):data_type=type(data);tag='tag:yaml.org,2002:python/object/apply:%s.%s'%(data_type.__module__,data_type.__name__);items=[[key,value]for(key,value)in data.items()];return self.represent_sequence(tag,[items])
